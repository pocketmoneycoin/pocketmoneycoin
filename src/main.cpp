...
#if defined(NDEBUG)
# error "Pocketmoneycoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Pocketmoneycoin Signed Message:\n";
...
{
    // Pocketmoneycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pocketmoneycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pocketmoneycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PocketmoneycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PocketmoneycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PocketmoneycoinMiner(CWallet *pwallet)
{
    printf("PocketmoneycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pocketmoneycoin-miner");
    ...
    try { loop {
        ...
        printf("Running PocketmoneycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PocketmoneycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PocketmoneycoinMiner, pwallet));
}
